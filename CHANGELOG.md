...
- **动态宽度系数选择**：模型配置界面中的宽度系数 (Alpha) 参数更改为下拉选框形式，并会根据 MobileNetV1 或 MobileNetV2 自动刷新两者的不同合法参数。
...
- **Dynamic Width Multipliers**: The Alpha parameter in the GUI configuration has been converted to a dropdown combo box which dynamically updates available multiplier options based on MobileNetV1 or MobileNetV2 selections.
