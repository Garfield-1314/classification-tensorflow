...
def evaluate_dataset(
    ...
    for i, (images, labels) in enumerate(tqdm(dataset, desc='测试进度')):
        if progress_cb and total_steps > 0:
            progress_cb(int((i / total_steps) * 100))
...
def plot_class_accuracies(class_names: List[str], accuracies: np.ndarray, save_dir: str, model_path: str) -> str:
    ...
    """
    plt.figure(figsize=(8, 5)) # 设定合理的尺寸，防止展示时太小
...
def main(model_path: str = MODEL_PATH, test_dir: str = TEST_DIR, progress_cb=None):
    ...
    class_names, correct_predictions, total_predictions = evaluate_dataset(
        interpreter, input_details, output_details, input_dtype, img_size, test_dir, BATCH_SIZE, progress_cb
    )
    ...
    # 释放 interpreter 阻止 TFLite 的释放错误警告
    interpreter = None

    # --- 动态添加: 如果有回调显示，这里可以返回保存地址与详细信息 ---
    return save_path, class_names, accuracies, correct_predictions, total_predictions
